...
class ToolManager:
    ...
    def install_tool(
        ...
        if handler is None:
            # Fallback: look for any callable matching the snake_case of tool_id
        ...
        # Step 7: install-complete STM event
        stm.record(
            source="system", type="internal",
            payload={
    ...
    def _fetch_descriptor_from_ltm(self, tool_id: str) -> ToolDescriptor:
        ...
        try:
            results = self.muninn.recall(f"tool {tool_id}", top_k=5)
            for r in results:
                try:
    ...
    def _recall_tool_schemas_from_ltm(
        ...
        try:
            results = self.muninn.recall("tool artifact descriptor", top_k=top_k)
            schemas = []
            for r in results:
                try:
                    data = json.loads(r.entry.content)
