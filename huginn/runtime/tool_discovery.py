...
class ToolManifest:
    ...
    mode        — "callable" (default): loaded on first call, returns a result.
                  "service": started once as a daemon thread; runs until stopped.
    ...
    subscriptions — (service output tools only) list of ActuationBus filter dicts.
                    ...
                    e.g. {"type": "output", "target": "speech", "complete": "chunk"}

    states      — (service tools) dict of configurable parameters and their defaults.
                  Written to HTM.states on activation as "{tool_id}.{param}".
                  e.g. {"speed": {"default": 1.0, "type": "float", "description": "..."}}
    """
    tool_id:            str
    title:              str
    ...
    def is_output_tool(self) -> bool:
        return self.direction in ("output", "io")

    @property
    def is_input_tool(self) -> bool:
        return self.direction in ("input", "io") or self.perception_capable
...
def parse_manifest(source_code: str, source_path: str = "") -> Optional[ToolManifest]:
    ...
    # Normalise subscriptions to list of dicts
    subs = data.get("subscriptions", [])
    if isinstance(subs, str):
        subs = []   # malformed — ignore

    # Normalise states to dict of {param: {default, type, description}}
