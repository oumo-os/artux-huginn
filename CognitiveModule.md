...
| `recall(query, top_k)` | Sagax | Hybrid recall. `query` is a `RecallQuery(operator, subject, topics, semantic_query, time_range, include_scars)` or a plain string (treated as `semantic_query` only). Topics give surgical exact-match; all signals combine into one ranked result. Returns `list[RecallResult]` each with `.entry`, `.score`, `.match_reasons`, `.sources`. |
